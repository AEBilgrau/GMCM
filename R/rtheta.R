...
#'
#' This function generates a random set parameters for the Gaussian mixture
...
#'   See details. Default \code{"old"} is to restore old behavior.
...
#'   follows. The new behaviour mimics the simulation scenarious in Friedman
#'   (1989).
#'   \itemize{
#'   \item{\code{pie}}{
#'     Generated by \eqn{m} draws of a chi-squared distribution with \eqn{3m}
#'     degrees of freedom divided by their sum. If \code{method = "old"}
#'     the uniform distribution is used instead.}
#'   \item{\code{mu}}{
#'     Generated by \eqn{m} iid \eqn{d}-dimensional zero-mean
#'     normal vectors with covariance \code{100I}. (Identical to old behavior)}
#'   \item{\code{sigma}}{
#'     Depending on \code{method}, the covariance matrices for each
#'     component are generated as follows.
#'     \itemize{
#'     \item{\code{"EqualSpherical"}}{
#'       The covariance matrices are the identity matrix and thus are all equal
#'       and spherical.}
#'     \item{\code{"UnequalSpherical"}}{
#'       The covariance matrices are scaled identity matrices. In component
#'       \eqn{h}, the covariance matrix is \eqn{hI}}
#'     \item{\code{"EqualEllipsoidal"}}{
#'       Highly elliptical covariance matrices which equal for all components.
#'       The squareroot of the \eqn{d} eigenvalues are chosen equidistantly on
#'       the interval \eqn{10} to \eqn{1} and a randomly (uniform) oriented
#'       orthonormal basis is chosen and used for all components.
#'     }
#'     \item{\code{"UnqualEllipsoidal"}}{
#'       Highly elliptical covariance matrices differenent for all components.
#'       The eigenvalues of the covariance matrices equal as in all components
#'       as in \code{"EqualEllipsoidal"}. However, they are all randomly
#'       (uniformly) oriented (unlike as described in Friedman (1989)).}
#'     \item{\code{"old"}}{
#'       The old behavior differs from \code{"EqualEllipsoidal"} by using the
#'       absolute value of \eqn{d} zero-mean iid normal eigenvalues
...
rtheta <- function (m = 3, d = 2,
  ...
  if (method == "old") {
    ...
    pie <- pie/sum(pie)
    mu    <- c(list(rep(0, d)),
               replicate(m - 1, rnorm(d, sd = 10), simplify = FALSE))
